...
try:
    ...
    from security import ComplianceFrameworkMapper, EnterpriseAuth
...
# MCP (Model Context Protocol) – Kali Linux AI tools
try:
...
class Config:
    ...
    def __init__(self, config_file: str = "config.ini"):
        ...
        self.ENABLE_DOCKER = self.config.getboolean("features", "enable_docker", fallback=False)
        self.ENABLE_KUBERNETES = self.config.getboolean("features", "enable_kubernetes", fallback=False)
        self.ENABLE_MCP = self.config.getboolean("features", "enable_mcp", fallback=True)
        # Cloud settings
...
class EnterpriseHackGPT:
        ...
        try:
            ...
            if MODULES_AVAILABLE:
                try:
                    ...
                    self.auth = None
                    self.console.print(f"[yellow]⚠[/yellow] Authentication not available: {e!s}")
            else:
                self.auth = None
            ...
            if MODULES_AVAILABLE:
                try:
                    ...
                    self.cache = None
                    self.console.print(f"[yellow]⚠[/yellow] Cache not available: {e!s}")
            else:
                self.cache = None
            ...
            if MODULES_AVAILABLE:
                try:
                    ...
                    self.processor = None
                    self.console.print(f"[yellow]⚠[/yellow] Parallel processing not available: {e!s}")
            else:
                self.processor = None
    ...
    def initialize_services(self):
        ...
        try:
            ...
            if self.config.ENABLE_DOCKER:
                ...
                self.docker_manager = None
                self.console.print("[yellow]⚠[/yellow] Docker disabled (enable_docker=false)")
            ...
            if self.config.ENABLE_KUBERNETES:
                ...
                self.k8s_manager = None
                self.console.print("[yellow]⚠[/yellow] Kubernetes disabled (enable_kubernetes=false)")
            ...
            if config.ENABLE_MCP and MCP_AVAILABLE:
                self.mcp_server = MCPKaliServer(config)
                self.console.print("[green]✓[/green] MCP Kali Linux server initialized")
            else:
                self.mcp_server = None
                if not MCP_AVAILABLE:
                    self.console.print("[yellow]⚠[/yellow] MCP not available (pip install 'mcp[cli]')")
                else:
                    self.console.print("[yellow]⚠[/yellow] MCP disabled (enable_mcp=false)")
    ...
    def create_fallback_ai(self):
        ...
        class FallbackAI:
            ...
            def analyze(self, context, data, phase="general"):
                return (
                    f"[Fallback AI Analysis]\nContext: {context}\n"
                    f"Phase: {phase}\nRecommendation: Please configure "
                    f"AI engine for detailed analysis."
                )
    ...
    def show_banner(self):
        ...
        components = [
            ...
            (
                "MCP Kali Server",
                "✓ Ready" if self.mcp_server else "⚠ Disabled",
                "MCP/HTTP",
            ),
        ]
    ...
    def show_main_menu(self):
        ...
        menu_options = [
            ...
            ("16", "MCP", "Start MCP Kali Linux Server"),
    ...
    def get_target_info(self):
        ...
        if self.compliance:
            ...
            compliance_framework = Prompt.ask("[cyan]Compliance framework[/cyan]", choices=frameworks, default="OWASP")
        else:
    ...
    def manage_cloud_services(self):
        ...
        choice = Prompt.ask("[cyan]Select option[/cyan]", choices=[opt[0] for opt in cloud_options])
    ...
    def deploy_hackgpt_stack(self):
        ...
        from cloud.docker_manager import ContainerConfig, ServiceDefinition
    ...
    def start_api_server(self):
        ...
        def start_pentest():
            try:
                ...
                thread = threading.Thread(target=self.run_full_enterprise_pentest, args=(target_info,))
                thread.start()

                return jsonify({"status": "started", "message": "Enterprise pentest initiated"})
            except Exception as e:
    ...
    def run(self):
        ...
        while True:
            try:
                self.show_main_menu()
                choice = Prompt.ask("[cyan]Select option[/cyan]", choices=[str(i) for i in range(17)])
    ...
    def launch_web_dashboard(self, host="0.0.0.0", port=8080):
        """Start the web dashboard."""
        if not getattr(self, "web_dashboard", None):
            try:
                self.console.print("[red]Web dashboard not available[/red]")
            except Exception:
                print("Web dashboard not available")
            return None

        # Allow config.ini overrides if present
        try:
            host = self.config.get("web", "host", fallback=host)
            port = self.config.getint("web", "port", fallback=port)
        except Exception:
            pass

        try:
            self.console.print(f"[cyan]Dashboard available at: http://{host}:{port}[/cyan]")
        except Exception:
            print(f"Dashboard available at: http://{host}:{port}")

        return self.web_dashboard.run(host=host, port=port)

    def start_mcp_server(self, background: bool = False):
        """Start the MCP Kali Linux server.

        Args:
            background: If True, run in background thread and return immediately.
        """
        if not getattr(self, "mcp_server", None):
            self.console.print("[red]MCP Kali Linux server not available.[/red]")
            self.console.print("[yellow]Install the MCP SDK: pip install 'mcp[cli]'[/yellow]")
            self.console.print("[yellow]Enable in config.ini: [features] enable_mcp = true[/yellow]")
            return

        self.console.print(
            Panel(
                f"[bold cyan]Starting MCP Kali Linux Server[/bold cyan]\n\n"
                f"Endpoint: [green]{self.mcp_server.endpoint_url}[/green]\n"
                f"Transport: Streamable HTTP (SSE fallback)\n\n"
                f"[dim]Connect Claude Desktop or any MCP-compatible AI to this endpoint.\n"
                f"Add to claude_desktop_config.json:[/dim]\n\n"
                f'  {{"mcpServers": {{"hackgpt-kali": {{"url": "{self.mcp_server.endpoint_url}"}}}}}}\n\n'
                f"[bold yellow]12 tools available:[/bold yellow] nmap, nikto, sqlmap, gobuster,\n"
                f"hydra, metasploit, whatweb, whois, hashcat, amass,\n"
                f"exploit_search, run_command\n\n"
                f"Press Ctrl+C to stop the server.",
                title="MCP Kali Linux Server",
                border_style="green",
            )
        )
        ...
        if background:
            ...
            self.console.print(f"[green]✓[/green] MCP server running in background on {self.mcp_server.endpoint_url}")
        else:
            try:
                self.mcp_server.start()
            except KeyboardInterrupt:
                self.mcp_server.stop()
                self.console.print("[yellow]MCP server stopped.[/yellow]")
...
class EnterpriseWebDashboard:
    """Enterprise web dashboard (Flask)"""
    ...
    def run(self, host=None, port=None):
        try:
            ...
            raise SystemExit(f"Flask not available: {e}. Try: pip install flask flask-cors") from e
        ...
        <head>
          ...
          <title>HackGPT Enterprise Dashboard</title>
          <style>
            body{font-family:system-ui,-apple-system,Segoe UI,Roboto,Ubuntu,"Helvetica Neue",Arial;
                 margin:40px;background:#fff;color:#111}
            h1{margin:0 0 18px 0;font-size:34px}
            .card{border:1px solid #e5e7eb;border-radius:14px;padding:22px;max-width:900px}
            ...
            code{background:#f3f4f6;padding:3px 7px;border-radius:8px}
          </style>
        </head>
        <body>
          <h1>HackGPT Enterprise Dashboard</h1>
          <div class="card">
            <div><strong>Status:</strong> <span class="pill">Ready</span></div>
            <div style="margin-top:10px">JSON status endpoint: <code>/api/status</code></div>
          ...
          <script>
            fetch('/api/status').then(r=>r.json()).then(d=>{
              ...
              const items = [
                ...
                ['web_dashboard', d.web_dashboard],
                ['realtime_dashboard', d.realtime_dashboard],
              ];
              for (const [k,v] of items){
                const el=document.createElement('div');
                el.className='pill';
                el.textContent = k+': '+String(v);
                meta.appendChild(el);
              }
            }).catch(()=>{});
          </script>
        ...
        def api_status():
            ...
            return jsonify(
                {
                    "status": "ok",
                    "target": getattr(hg, "target", None),
                    "assessment_type": getattr(hg, "assessment_type", None),
                    "web_dashboard": True,
                    "realtime_dashboard": bool(getattr(hg, "realtime_dashboard", None)),
                }
            )
...
def main():
    ...
    parser.add_argument("--mcp", action="store_true", help="Start MCP Kali Linux server only")
