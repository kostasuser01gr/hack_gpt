...
class CacheEntry:
    ...
    def is_expired(self) -> bool:
        return bool(self.expires_at and datetime.now(tz=timezone.utc) > self.expires_at)
        ...
        try:
            with self.lock:
                ...
                if ttl is not None:
                    ...
                    expires_at = datetime.now(tz=timezone.utc) + timedelta(seconds=self.default_ttl)
    ...
    def __init__(
        ...
        try:
            if sentinel_hosts:
                ...
                self.client = sentinel.master_for(sentinel_service, socket_timeout=0.1, password=password, db=db)
            else:
...
class CacheManager:
    ...
    def __init__(
        ...
        self.function_cache: Any = weakref.WeakKeyDictionary()
    ...
    def exists(self, key: str) -> bool:
        ...
        return self.l1_cache.exists(key) or bool(self.l2_cache and self.l2_cache.exists(key))
    ...
    def cache_result(self, ttl: int | None = None, key_func: Callable | None = None):
        ...
        def decorator(func):
            def wrapper(*args, **kwargs):
                # Generate cache key
                cache_key = key_func(*args, **kwargs) if key_func else self._generate_cache_key(func, args, kwargs)
