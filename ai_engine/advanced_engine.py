...
try:
    ...
    from transformers import AutoModel, AutoTokenizer, pipeline  # noqa: F401
...
class ContextManager:
    ...
    def get_relevant_context(self, query: str, context_type: str | None = None) -> dict[str, Any]:
...
class VulnerabilityCorrelator:
    ...
    def _calculate_compound_risk(self, attack_chains: list[dict[str, Any]]) -> dict[int, float]:
        ...
        for i, chain in enumerate(attack_chains):
            ...
            impact_score = sum(1 for keyword in impact_keywords if keyword in chain.get("impact", "").lower())
...
class AdvancedAIEngine:
    ...
    def analyze_with_context(self, data: str, phase: str, context_type: str = "general") -> AnalysisResult:
        ...
        try:
            ...
            pattern_info = [
                f"{p[0].pattern_type}: {p[0].pattern_id} (confidence: {p[1]:.2f})" for p in detected_patterns
            ]
            ...
            # Get AI analysis
            analysis_text = self._analyze_local(prompt) if self.local_mode else self._analyze_openai(prompt)
...
"""
    ...
    def _analyze_openai(self, prompt: str) -> str:
        ...
        try:
            ...
            return f"AI Analysis Error: {e!s}"
    ...
    def _structure_analysis(self, analysis_text: str, patterns: list[tuple]) -> AnalysisResult:
        ...
        for line in lines:
            ...
            if "recommendation" in line.lower() and (":" in line or "#" in line):
                ...
                continue
            if "next" in line.lower() and "action" in line.lower() and (":" in line or "#" in line):
                current_section = "actions"
                continue
            if "confidence" in line.lower() and (":" in line or "#" in line):
