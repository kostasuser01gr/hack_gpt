...
class DatabaseManager:
    ...
    def update_session_status(self, session_id: str, status: str, user_id: str):
        ...
        with self.get_session() as session:
            ...
            if pentest_session:
                ...
                if status == "completed":
                    pentest_session.completed_at = datetime.now(tz=timezone.utc)
    ...
    def update_phase_result(
        ...
        with self.get_session() as session:
            ...
            if phase_result:
                ...
                phase_result.completed_at = completed_at or datetime.now(tz=timezone.utc)
                if execution_time:
    ...
    # User management
    def create_user(self, username: str, email: str, password_hash: str, role: str = "analyst") -> Any:
        """Create a new user"""
        with self.get_session() as session:
    ...
    def get_ai_context(self, session_id: str, context_type: str | None = None) -> list[AIContext]:
    ...
    def get_historical_trends(self, days: int = 30) -> dict[str, Any]:
        ...
        with self.get_session() as session:
            ...
            # Session trends
            session_count = session.query(PentestSession).filter(PentestSession.created_at >= cutoff_date).count()

            # Vulnerability trends
            vuln_trends = (
                session.query(Vulnerability.severity, func.count(Vulnerability.id).label("count"))
