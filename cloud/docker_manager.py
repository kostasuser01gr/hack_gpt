...
try:
    import docker
    from docker.errors import APIError, BuildError, DockerException  # noqa: F401
...
class DockerManager:
    ...
    def create_network(self, network_name: str, driver: str = "bridge") -> bool:
        ...
        try:
            ...
            # Create the network
            self.client.networks.create(network_name, driver=driver, check_duplicate=True)
    ...
    def create_volume(self, volume_name: str, driver: str = "local") -> bool:
        ...
        try:
            ...
            # Create the volume
            self.client.volumes.create(name=volume_name, driver=driver)
        ...
        for service in services:
            ...
            if config.ports:
                service_config["ports"] = [
                    f"{host_port}:{container_port}" for container_port, host_port in config.ports.items()
                ]

            # Add volumes
            if config.volumes:
                service_config["volumes"] = [
                    f"{host_path}:{container_path}" for container_path, host_path in config.volumes.items()
                ]

                # Add named volumes to the volumes section
                for _container_path, host_path in config.volumes.items():
            ...
            if config.memory_limit or config.cpu_limit:
                ...
                if config.memory_limit:
                    deploy_config["resources"] = {"limits": {"memory": config.memory_limit}}
                if config.cpu_limit:
