...
class ServiceRegistry:
    ...
    def __init__(self, backend: str = "memory", **kwargs):
        ...
        self.services: dict[str, list[ServiceInstance]] = {}  # In-memory storage
        ...
        if backend == "consul" and CONSUL_AVAILABLE:
            self.consul_client = consul.Consul(host=kwargs.get("host", "localhost"), port=kwargs.get("port", 8500))
        elif backend == "etcd" and ETCD_AVAILABLE:
            self.etcd_client = etcd3.client(host=kwargs.get("host", "localhost"), port=kwargs.get("port", 2379))
        else:
            self.backend = "memory"
    ...
    def register_service(self, service: ServiceInstance) -> bool:
        ...
        try:
            service.last_heartbeat = datetime.now(tz=timezone.utc)
    ...
    def discover_services(self, service_name: str, healthy_only: bool = True) -> list[ServiceInstance]:
    ...
    def heartbeat(self, service_name: str, instance_id: str) -> bool:
        ...
        try:
            if self.backend == "memory" and service_name in self.services:
    ...
    def _register_memory(self, service: ServiceInstance) -> bool:
        ...
        self.services[service.service_name] = [
            s for s in self.services[service.service_name] if s.instance_id != service.instance_id
        ]
    ...
    def _deregister_memory(self, service_name: str, instance_id: str) -> bool:
        ...
        if service_name in self.services:
            ...
            self.services[service_name] = [s for s in self.services[service_name] if s.instance_id != instance_id]
    ...
    def _discover_consul(self, service_name: str, healthy_only: bool) -> list[ServiceInstance]:
        ...
        try:
            _, services = self.consul_client.health.service(service_name, passing=healthy_only)
            ...
            for service_data in services:
                ...
                status = "healthy"
                if health_info and not all(check["Status"] == "passing" for check in health_info):
                    status = "unhealthy"
    ...
    def _list_consul(self) -> dict[str, list[ServiceInstance]]:
        ...
        try:
            ...
            service_map: dict[str, list[ServiceInstance]] = {}
    ...
    def get_health_status(self, service_name: str | None = None) -> dict[str, Any]:
