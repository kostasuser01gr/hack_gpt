...
class ChartGenerator:
    ...
    def create_timeline_chart(self, sessions: list[dict[str, Any]]) -> str:
        ...
        try:
            ...
            for session in sessions[-20:]:  # Last 20 sessions
                created_at = session.get("created_at", datetime.now(tz=timezone.utc).isoformat())
            ...
            # Plot timeline
            ax.scatter(dates, range(len(dates)), alpha=0.7, s=100)  # type: ignore[arg-type]

            # Add labels
            for i, (date, name) in enumerate(zip(dates, session_names)):
                ax.annotate(
                    name,
                    (date, i),  # type: ignore[arg-type]
                    xytext=(10, 0),
    ...
    def create_risk_heatmap(self, vulnerabilities: list[dict[str, Any]]) -> str:
        ...
        try:
            ...
            df_data = []
            for _category, severities in categories.items():
                df_data.append(
                    [
                        severities["critical"],
                        severities["high"],
    ...
    def _analyze_target_trends(self, sessions: list[dict[str, Any]], timeframe: str) -> dict[str, Any]:
        ...
        for period, period_sessions in time_groups.items():
            ...
            for session in period_sessions:
                ...
                elif target.startswith(("192.168.", "10.")):
                    target_type = "internal"
                else:
                    target_type = "unknown"

                target_stats["target_types"][target_type] = target_stats["target_types"].get(target_type, 0) + 1
...
<body>
    ...
    def generate_executive_report(self, sessions: list[dict[str, Any]], timeframe: str = "monthly") -> dict[str, Any]:
        ...
        if all_vulnerabilities:
            charts["severity_distribution"] = self.chart_generator.create_vulnerability_severity_chart(
                all_vulnerabilities
            )
        ...
        return {
            ...
            "metrics": {
                ...
                "unique_targets": len({s.get("target", "unknown") for s in sessions}),
            },
            "trends": trends,
            "insights": insights,
            "charts": charts,
    ...
    def _generate_executive_insights(
        ...
        if len(vuln_trends) >= 2:
            ...
            if previous_avg > 0:
                ...
                trend_direction = (
                    "increased" if change_percent > 5 else "decreased" if change_percent < -5 else "remained stable"
                )
                ...
                insights.append(
                    {
                        ...
                        "impact": (
                            "high" if abs(change_percent) > 20 else "medium" if abs(change_percent) > 5 else "low"
                        ),
                    }
                )
    ...
    def _generate_strategic_recommendations(self, insights: list[dict[str, Any]], trends: dict[str, Any]) -> list[str]:
    ...
    def _export_html_report(self, report_data: dict[str, Any]) -> str:
        ...
        try:
            ...
            return f"<html><body><h1>Error generating report: {e!s}</h1></body></html>"
    ...
    def _export_pdf_report(self, report_data: dict[str, Any]) -> str:
        ...
        try:
            ...
            pdf_path = f"/tmp/report_{datetime.now(tz=timezone.utc).strftime('%Y%m%d_%H%M%S')}.pdf"
