...
try:
    import websockets
    from aiohttp import WSMsgType, web  # noqa: F401
except ImportError as e:
    logging.warning(f"WebSocket dependencies not available: {e}")

# Use Any for WebSocket protocol type since the API varies across versions
...
class MetricsCollector:
    ...
    def _process_metrics_for_alerts(self, session_id: str, metrics: dict[str, Any]):
        ...
        for rule in self.alert_rules:
            ...
            triggered = False
            if (
                (rule.condition == ">" and float(metric_value) > rule.threshold)
                or (rule.condition == "<" and float(metric_value) < rule.threshold)
                or (rule.condition == "==" and float(metric_value) == rule.threshold)
                or (rule.condition == "!=" and float(metric_value) != rule.threshold)
            ):
                triggered = True
...
class WebSocketManager:
    ...
    async def unregister_connection(self, websocket: WebSocketServerProtocol):
        ...
        for _session_id, subscribers in self.session_subscriptions.items():
    ...
    async def process_client_message(self, websocket: WebSocketServerProtocol, data: dict[str, Any]):
        ...
        if message_type == "subscribe_session":
            ...
            if session_id:
                ...
                await websocket.send(json.dumps({"type": "global_metrics", "data": metrics}))
    ...
    async def subscribe_to_session(self, websocket: WebSocketServerProtocol, session_id: str):
        ...
        await websocket.send(json.dumps({"type": "subscription_confirmed", "session_id": session_id}))
...
class RealTimeDashboard:
    ...
    async def start_server(self):
        ...
        server = await websockets.serve(self.websocket_manager.register_connection, self.host, self.port)
    ...
    async def periodic_updates(self):
        ...
        while self.running:
            try:
                ...
                await self.websocket_manager.broadcast_global({"type": "global_metrics_update", "data": global_metrics})

                # Send session-specific updates
                for session_id in self.websocket_manager.session_subscriptions:
